...
int main(int argc, char* argv[]) {
  ...
  for (int i_config = 1;; ++i_config) {
    for (double theta = 0.0; theta < 1.01; theta += 0.05) {
      read_gauge_field(U, base_name, i_config);

      // Get eigenvalues of reduced matrix P (expensive part)
      // BEFORE: multiply gauge link at ix3=0 by e^{i 2\pi theta}
      Eigen::MatrixXcd P_evals = D.P_eigenvalues(U, theta);
      ...
      // eigenvalues, or add random 1e-15 values, and see how both of these
      // affect the output of extended precision calculations below
      int k_max = 6 * U.VOL3;
      ...
      for (int n = 0; n < k_max; ++n) {
        ...
        for (int k = 1; k <= n + 1; ++k) {
          c_new_re[k] =
              e_re * c_old_re[k] - e_im * c_old_im[k] + c_old_re[k - 1];
          c_new_im[k] =
              e_re * c_old_im[k] + e_im * c_old_re[k] + c_old_im[k - 1];
        }
        c_old_re = c_new_re;
        c_old_im = c_new_im;
      }
      ...
      // Q = k - k_max/2
      std::cout << "# Coeffs (Q, theta, ZC_re, ZC_im, phase): " << std::endl;
      for (int k = 0; k <= k_max; ++k) {
        int Q = k - k_max / 2;
        mpfr::mpreal phase = atan2(c_new_im[k], c_new_re[k]);
        std::cout << i_config << "\t" << Q << "\t" << theta << "\t"
                  << c_new_re[k] / det << "\t" << c_new_im[k] / det << "\t"
                  << phase << std::endl;
      }
    }
  }
