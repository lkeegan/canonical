...
lattice::lattice(int L0, int L1, int L2, int L3, bool isEO)
    : neighbours_up_(4 * L0 * L1 * L2 * L3),
      neighbours_dn_(4 * L0 * L1 * L2 * L3),
      L0(L0),
      L1(L1),
      L2(L2),
      L3(L3),
...
int lattice::pbcs(int x, int L) const { return (x + 2 * L) % L; }
...
int lattice::index_LEXI(int x0, int x1, int x2, int x3) const {
  return pbcs(x0, L0) + L0 * pbcs(x1, L1) + L0 * L1 * pbcs(x2, L2) +
         L0 * L1 * L2 * pbcs(x3, L3);
}
...
int lattice::index_EO(int x0, int x1, int x2, int x3) const {
  ...
  int eo_offset =
      (V / 2) *
      ((pbcs(x0, L0) + pbcs(x1, L1) + pbcs(x2, L2) + pbcs(x3, L3)) % 2);
